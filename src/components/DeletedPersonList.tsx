...
const PersonList = () => {
  ...
  const restorePersons = async (ids: number[]) => {
    if (ids.length) {
      await api.post("/restore-person", { ids });
      initializeData();
    }
  };
  const bulkRestore = async () => {
    setLoading(true);
    await restorePersons(selectedValues);
    setSelectedValues([]);
    setLoading(false);
  };
  ...
  const getPersons = async (newSkip: number, q: string) => {
    setLoading(true);
    await api
      .post("/deleted-list", { skip: newSkip, limit, q })
      .then((response) => {
  ...
  const getCount = async (searchText: string) => {
    await api.post("/deleted-count", { q: searchText }).then((response) => {
